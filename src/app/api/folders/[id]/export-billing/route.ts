...
export async function GET(
  ...
  try {
    ...
    // CSV Header
    const headers = ["Date", "Venue Name", "User Name", "User Email", "Duration (mins)", "Billing Code", "Confirmation ID", "Mock Expense ($)"];
    
    const rows = bookings.map(booking => {
      const userName = `${booking.user.firstName || ''} ${booking.user.lastName || ''}`.trim() || 'Anonymous';
